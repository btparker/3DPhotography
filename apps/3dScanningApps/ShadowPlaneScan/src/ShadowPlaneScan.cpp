#include "ShadowPlaneScan.h"
using namespace cv;

//--------------------------------------------------------------
void ShadowPlaneScan::setup(){
...
void ShadowPlaneScan::assertPoint(ofPoint pt){
...
*/
void ShadowPlaneScan::loadSettings(){
	/* Load settings file */
...
*/
void ShadowPlaneScan::saveSettings(){
...
//--------------------------------------------------------------
void ShadowPlaneScan::clearSettings(){
...
void ShadowPlaneScan::camCalUpdate(){
...
void ShadowPlaneScan::setupUpdate(){
    ofPoint imagePts[8];
    ofPoint objectPts[8];
    ofPoint pixelCoord;
    ofxRay3d ray;
...
ofPoint ShadowPlaneScan::pt3DToPixel(const CvMat* intrinsicMat, const CvMat* extrinsicMat, ofPoint pt3D){
...
void ShadowPlaneScan::convertOfPointsToCvMat(ofPoint *pts, int dimensions, int size, CvMat* output){
    if(pts == NULL){
        return;
    }
...
void ShadowPlaneScan::captureUpdate(){
...
void ShadowPlaneScan::processingUpdate(){
    ofPoint pts[3];
...
void ShadowPlaneScan::points3dUpdate(){
...
void ShadowPlaneScan::writePointsToFile(int numPoints){
...
void ShadowPlaneScan::camCalDraw(){

}

void ShadowPlaneScan::setupDraw(){
    drawSectionRectangles();
    drawMarkerPoints();
}

void ShadowPlaneScan::captureDraw(){
    drawSectionRectangles();
    drawMarkerPoints();
}

void ShadowPlaneScan::processingDraw(){
    drawSectionRectangles();
    drawMarkerPoints();
...
/**
    ...
    @returns bool
*/
bool ShadowPlaneScan::isPointInRegion(ofPoint pt, ofRectangle roi){
    if(pt.x == -1 || pt.y == -1){
        return false;
    }
    return  pt.x >= roi.x && 
...
*/
void ShadowPlaneScan::drawSectionRectangles(){
    ofSetLineWidth(2);
    ofNoFill();
...
*/
void ShadowPlaneScan::drawMarkerPoints(){
...
*/
ofxLine2d ShadowPlaneScan::computeLineFromZeroCrossings(ofxCvGrayscaleImage img, ofRectangle roi){
    int roi_x0 = roi.x;
    int roi_y0 = roi.y;
...
/**
    ...
    @returns ofPoint
*/
ofPoint ShadowPlaneScan::getNearestCorner(ofxCvGrayscaleImage img, int windowSize, int x, int y){
    Point2f cvPt(x,y);
    vector<Point2f> cvPts;
    cvPts.resize(1,cvPt);
	cv::Size winSize = cv::Size( windowSize, windowSize );
...
void ShadowPlaneScan::computeExtrinsicMatrix(ofPoint *objectPoints, ofPoint *imagePoints, CvMat* cameraMatrix, CvMat* distCoeffs){
...
ofxRay3d ShadowPlaneScan::pixelToRay(const CvMat* intrinsicMat, const CvMat* extrinsicMat, ofPoint imagePt){
...
// ofPoint ShadowPlaneScan::rayPlaneIntersection(ofPoint planePt, ofVec3f planeNormal, ofPoint rayOrigin, ofVec3f rayDirection){
...
//--------------------------------------------------------------
void ShadowPlaneScan::keyPressed(int key){
    switch(key){
        case 32: //SPACEBAR
...
//--------------------------------------------------------------
void ShadowPlaneScan::keyReleased(int key){

}

//--------------------------------------------------------------
void ShadowPlaneScan::mouseMoved(int x, int y ){

}

//--------------------------------------------------------------
void ShadowPlaneScan::mouseDragged(int x, int y, int button){
    if(programState == SETUP){
        switch(setupSubState){
            case TOP_SECTION:
...
//--------------------------------------------------------------
void ShadowPlaneScan::mousePressed(int x, int y, int button){
     if(programState == SETUP){
        switch(setupSubState){
            case TOP_SECTION:
...
//--------------------------------------------------------------
void ShadowPlaneScan::mouseReleased(int x, int y, int button){
    /*
...
//--------------------------------------------------------------
void ShadowPlaneScan::windowResized(int w, int h){

}

//--------------------------------------------------------------
void ShadowPlaneScan::gotMessage(ofMessage msg){

}

//--------------------------------------------------------------
void ShadowPlaneScan::dragEvent(ofDragInfo dragInfo){ 

}

//--------------------------------------------------------------
ofxCvGrayscaleImage ShadowPlaneScan::computeGradientImage(ofxCvGrayscaleImage &input, int direction){
    int sum = 0;
    int sumX = 0;
    int sumY = 0;
...
ofxLine3d ShadowPlaneScan::projectLineOntoPlane(ofxLine2d line, ofxPlane plane, const CvMat* intrinsicMat, const CvMat* extrinsicMat){
...
float ShadowPlaneScan::getFrameFromColor(ofColor color){
...
bool ShadowPlaneScan::isPlaneAtFrameIndex(float fi){
...
ofxPlane ShadowPlaneScan::getPlaneFromFrameIndex(float fi){
    int discreteFrameIndex = (int)fi;
    float interpolateFrameValue = fi - discreteFrameIndex;
    if(discreteFrameIndex > 0){
...
void ShadowPlaneScan::drawPointCloud() {
    ofBackground(0);
    ofMesh mesh;
    int step = 1;
    ofxPlane plane;
...
ofxCvGrayscaleImage ShadowPlaneScan::computeBinCodeImage(int w, int h, int power, int type){
